...
the Unix shell. You will see lots of curly braces in the upcoming material, and
most of those are serving a different purpose.)
...
(print the shell commands) by putting them together with `-np`:
```sh
...
actual file names and they appear in the first block of the Snakefile.
Because of this, we could just run `snakemake -np` without
...
awesome-trimmed output for all the other samples too.  The naive approach would be to
...
_wildcard phrase_. The wildcards in that wildcard phrase
is`{sample}`.  Each of those wildcard in a wildcard phrase can take
...
snakemake -np results/awesome/002_R1.fq results/awesome/002_R2.fq
```
This time, Snakemake finds the rule to create those output files and it knows
...
like this.  Again, we note that these curly braces are different from the curly 
braces you have seen in the Snakemake/Python wildcards.  These are just
Unix brace-expansion curly braces.
```sh
# do this on your unix terminal in directory smk-tut
snakemake -np results/awesome/00{1..5}_R{1,2}.fq
```
Again, we note that these curly braces are different from the curly 
braces you have seen in the Snakemake/Python wildcards.  These are just
...
value `{sample} = 002` to the input, but didn't find the appropriate input
files:
```
Building DAG of jobs...
...
The reason for this is so that we explore the `unpack()` function later.
...
not only short of gibberishy, but also is not consistent across the
...
```
In fact, this file is super important.  It tells you how nice tidy sample names like `s001` and `s002` correspond to the
...
Here is where we are going: we are going to read the `samples.csv` table into Python variable that will
...
package.  If you are new to Python, you can think of Pandas as the Python equivalent of R's 'dplyr' package.
Let's play with it a little bit to familiarize ourselves with it...

### Playing with Pandas

The Pandas package is inluded in your `snakemake` conda environment, and we are going to play with
it a little bit, so, open a new terminal window, activate your snakemake conda environment and then
...
python
```
Now, you have an interactive Python session in that terminal window. And you can do this
to import the Pandas package into the session and read in the CSV file and print it:
```python
...
# this reads the CSV file and sets an index using the values is the "sample" column.
...
```
OK, `samples_table` is just a table of data.  Importantly, it is indexed by the sample name (s001, s002, etc.)  This
...
# get the paths to fastq file 1 for sample s003:
samples_table.loc["s003", "fastq1"]
```
That is quite tasty!  The `.loc` is method in Pandas to return the location (i.e., the row)
...
path of a FASTQ file.  This is just the sort of thing we need, but we have to wrap it up
into a Snakemake _input function_.

Before we go big and do that in our Snakefile, we are going to spoof
it in our interactive Python session.  This turns out to be a great way to test and develop
and test your Snakemake input functions.

### Spoofing a Snakemake input function
...
We can use our Python interactive session to create a fictitious `wildcards` object and give
