...
Currently very incomplete.  Mostly throwing down things that are a little
more obscure that I want to learn about at some point, so would be good
to teach them.
...
Note that this happens at multiple steps in the process.  I think it would
be good to address all this once the main steps of the workflow have been 
explained.  (i.e. a separate section of all the different QC steps that go
on in between the main steps.)

It will behoove us to talk about the pitfalls of each of the main steps,
of course, so that it will be clear why we are doing these QC steps.
