...
3. Use your alias `myjobs` (if you have created it) to see what jobs you have running.
    ...
    ```
4. Navigate to your scratch directory.  On SUMMIT there is a symbolic link called `scratch` in your home directory.
    ```{sh, eval=FALSE}
    cd scratch
    ```
4. Make a directory called `chinook-play` and `cd` into it:
    ```{sh, eval=FALSE}
    mkdir chinook-play
    cd chinook-play/
    ```
...
6. Now, use rclone to list the files in the folder I shared with you:
    ```{sh, eval=FALSE}
    rclone ls --drive-shared-with-me gdrive-pers:CSU-con-gen-2020
    ```
...
7. After you get the above command to work, you should see:
    ```{sh, eval=FALSE}
    416265040 tiny-genomic-data.zip
    179506752 big-fastq-play.zip
    1212848720 pre-indexed-chinook-genome/GCA_002872995.1_Otsh_v1.0_genomic.fna.gz.sa
      4599378 pre-indexed-chinook-genome/GCA_002872995.1_Otsh_v1.0_genomic.fna.gz.amb
      2118234 pre-indexed-chinook-genome/GCA_002872995.1_Otsh_v1.0_genomic.fna.gz.ann
    606424334 pre-indexed-chinook-genome/GCA_002872995.1_Otsh_v1.0_genomic.fna.gz.pac
    2425697412 pre-indexed-chinook-genome/GCA_002872995.1_Otsh_v1.0_genomic.fna.gz.bwt
    737881835 pre-indexed-chinook-genome/GCA_002872995.1_Otsh_v1.0_genomic.fna.gz
    ```
8. Use rclone to download `big-fastq-play.zip` (about 170 MB):
    ```{sh, eval=FALSE}
...
9. Create a directory called `fastq` and move the two paired-end FASTQ files from `big-fastq-play` into it:
    ...
    unzip big-fastq-play.zip
    mkdir fastq 
    big-fastq-play/data/Battle_Creek_01_chinook_R* fastq/
...
13. Index the genome with `bwa` in order to align reads to it:
    ...
    - Navigate to `~/scratch/cd tiny-genomic-data/genome/` and list the files there.  These new files
...
instead, here is what we will do: 
    ...
    result in the `chinook-play` directory inside a directory called `chinook-genome-idx`.
    ```{sh, eval=FALSE}
