...
the environment is activated, all that is really happening is that `conda` is manipulating your
...
4. Navigate to your scratch directory.  On SUMMIT there is a symbolic link called `scratch` in your home directory.
    ...
    cd scratch
    ```
5. Make sure you have followed the link in your email to the shared google drive folder I sent you last night.
...
7. After you get the above command to work, you should see:
    ...
    ```
8. Use rclone to download `tiny-genomic-data.zip` (about 416 MB):
    ```{sh, eval=FALSE}
    rclone copy -P  --drive-shared-with-me gdrive-pers:CSU-con-gen-2020/tiny-genomic-data.zip ./
    ```
9. Unzip the file you just downloaded:
    ```{sh, eval=FALSE}
    unzip tiny-genomic-data.zip
    ```
10. Change directories into `tiny-genomic-data`
    ```{sh, eval=FALSE}
    unzip tiny-genomic-data.zip
    ```
